CREATE OR REPLACE PROCEDURE SILVER.SP_SILVER_TRANSACTIONS_CLIENT_C()
RETURNS STRING
LANGUAGE SQL
AS
$$
BEGIN
    ...
    USING (
        ...
        SELECT 
            ...
            AMOUNT_CURRENCY,
            SOURCE_SYSTEM,
            HASH(ORDER_ID, ORDER_DATE, CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, NOTE, PRODUCT_ID, PRODUCT_DESCRIPTION, QUANTITY, UNIT_PRICE, UNIT_PRICE_CURRENCY, PAYMENT_METHOD, AMOUNT, AMOUNT_CURRENCY) as TRACKINGHASH,
            INGESTED_AT
        FROM extracted_data
    ) source
